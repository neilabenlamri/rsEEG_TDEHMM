...
def pack_sequences_and_groups(
    ...
    if isinstance(data, dict):
        ...
        return Data(seqs, store_dir="/mnt/uphummel/scratch/nbenlamr/tmp/osl_data_store"), np.asarray(groups, int), True
    else:
        # legacy single-subject: group id = 0 for all sequences
        seqs = list(data)
        groups = np.zeros(len(seqs), dtype=int)
        return Data(seqs, store_dir="/mnt/uphummel/scratch/nbenlamr/tmp/osl_data_store"), groups, False
...
def subset_Data(data_all: Data, seq_idx: np.ndarray) -> Data:
    ...
    return Data(seqs, store_dir="/mnt/uphummel/scratch/nbenlamr/tmp/osl_data_store")
# === end Functions === #
...
data_raw = Data(
    all_trials,
    store_dir="/mnt/uphummel/scratch/nbenlamr/tmp/osl_data_store_feature_extraction"
)
...
OUT = Path("/mnt/uphummel/scratch/nbenlamr/osl_results/spectra/AcuteStroke")
OUT.mkdir(parents=True, exist_ok=True)

np.save(OUT / "f_v003_downsampl_0Hz.npy", f)
np.save(OUT / "psd_v003_downsampl_0Hz.npy", psd)
np.save(OUT / "w_v003_downsampl.npy", w)
np.save(OUT / "coh_v003_downsampl.npy", coh)

coh_clean_path = OUT / "coh_clean_v003_downsampl.npy"
coh_clean = np.lib.format.open_memmap(
    coh_clean_path,
    mode="w+",
    dtype=coh.dtype,
    shape=coh.shape,
...
for i in range(coh.shape[0]):
    coh_clean[i] = np.clip(
        np.nan_to_num(coh[i], nan=0.0, posinf=0.0, neginf=0.0),
        0,
        1,
    )

    if i % 50 == 0:
        print(f"[INFO] Cleaned coherence {i}/{coh.shape[0]}", flush=True)
